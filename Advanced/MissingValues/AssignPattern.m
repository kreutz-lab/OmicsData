...
function O = AssignPattern(O,out,npat,expand)
...
if ~exist('npat','var') || isempty(npat)
    if size(O,1)*size(O,2)<50000
        npat = 20;
    elseif size(O,1)*size(O,2)<100000
        npat = 10;
    else
        npat = 5;
    end
...
Os = O(1:size(O,1)-sum(all(isnan(ori),2)),:); % delete all empty rows
...
for i=1:npat
    ...
    brow = brow(ceil(rand(size(X,2)+1-length(b),1)*length(brow)));
    brow = sort(brow);
    ...
    p = reshape(yhat,length(brow),size(Os,2));
    if size(p,1)<size(Os,1)
        p = [p; p(randsample(size(p,1),size(Os,1)-size(p,1)),:)];
    end
    ...
    drin = find(all(isnan(dat_mis),2) & ~all(isnan(comps),2));
    if ~isempty(drin)
        for d = 1:length(drin)
            a = find(~isnan(comps(drin(d),:)));
            dat_mis(drin(d),a(randsample(length(a),1))) = comps(drin(d),a(randsample(length(a),1)));
        end
    end

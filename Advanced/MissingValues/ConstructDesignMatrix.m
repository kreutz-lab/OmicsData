% [X,y,type,bnames] = ConstructDesignMatrix(O,[out,bio])
%
% Construct design matrix X with the factorial row/col predictors and the
% mean intensity
%
% Inputs:
% O - omics data object
...
%
% Example:
% [X,y] = ConstructDesignMatrix(O);
% out = LogReg(X,y);

function [X,y,type,bnames] = ConstructDesignMatrix(O,out,bio)
